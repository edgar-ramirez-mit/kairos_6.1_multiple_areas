...
def validate_sources(active_service_configs):
    ...
    for camera_mac in active_service_configs:
        for parameter in active_service_configs[camera_mac]:
            if "source" == parameter:
                index = 7
                if active_service_configs[camera_mac][parameter][:4] == "file":
                    index = 8

                if active_service_configs[camera_mac][parameter][:index] not in com.SOURCE_PATTERNS:
                    com.log_error("Configuration error - Source should be one of this values: {}{}".
                                  format(com.SOURCE_PATTERNS,active_service_configs[camera_mac][parameter][:index]))
                if active_service_configs[camera_mac][parameter][:index] == com.SOURCE_PATTERNS[0]:
                ...
                '''
                if active_service_configs[camera_mac][parameter][:7] not in com.SOURCE_PATTERNS:
                    com.log_error("Configuration error - Source should be one of this values: {} but is {}".
                                  format(com.SOURCE_PATTERNS, active_service_configs[camera_mac][parameter][:8]))
